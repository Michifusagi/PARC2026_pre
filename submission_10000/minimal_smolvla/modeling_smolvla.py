...
class SmolVLAPolicy(nn.Module):
    ...
    @classmethod
    def from_pretrained(cls, model_dir: str | Path, device: str = "cpu", strict: bool = False):
        model_dir = Path(model_dir)
        config = SmolVLAConfig.from_pretrained(model_dir, device=device)
        policy = cls(config)
        ...
        if missing_keys:
            print(f"SmolVLA load: missing keys: {len(missing_keys)}")
        if unexpected_keys:
            print(f"SmolVLA load: unexpected keys: {len(unexpected_keys)}")
        policy.to(device)
        policy.eval()
        return policy
...
class VLAFlowMatching(nn.Module):
    ...
    def __init__(self, config: SmolVLAConfig, rtc_processor=None):
        super().__init__()
        self.config = config
